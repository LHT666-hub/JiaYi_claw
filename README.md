...
- 我的：健康记录、家属授权、帮助与反馈、通知偏好、隐私授权和账号注销。
...
- 医疗健康信息、AI、家属代办和通知分别记录授权，可撤回；账号必需的基础政策通过注销流程终止，不用一个容易误触的开关处理。
- 居民反馈按机构和社区隔离，重复提交被幂等键拦截；工作人员处理状态和结论写入事件记录。
...
- `verify:operations`：以生产构建验证内容到期下架、访客公开资料与原文防篡改、反馈幂等与审计、基础隐私边界、cron 鉴权、通知幂等和第 5 次失败进入死信。
- `verify:assistant-continuity`：验证居民/家属会话隔离、未授权拒绝、无原文存储和清除级联。
- `verify:rls`：70 条真实 JWT 断言，覆盖跨居民、可撤销家属授权、健康记录代录与数值约束、居民反馈、跨社区、跨机构、临床写入、经办认领和旧表隐私边界。
...
- `verify:wechat-package`：检查小程序页面产物、客服反馈闭环、包体、录音隐私声明、搜索屏蔽和开发入口泄漏。
