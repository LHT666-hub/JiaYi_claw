...
export default function MePage() {
  ...
  return <PhoneShell showBottomNav><div className="space-y-4 px-4 pb-8 pt-7">
    ...
    <section className="mt-5 rounded-[28px] border border-line bg-white p-4 shadow-[0_14px_34px_rgba(16,42,67,0.06)]"><div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[16px] bg-health-soft text-sage"><Stethoscope className="h-5 w-5" /></span><div><p className="text-xs text-navy/45">已绑定家医网络</p><h2 className="mt-1 font-semibold text-navy">{data?.network?.name ?? "尚未绑定"}</h2><p className="mt-2 text-sm text-navy/55">{data?.network?.community?.name ?? "请联系社区工作人员完成绑定"}</p><p className="mt-1 text-xs text-navy/40">协作机构 {data?.network?.institutions?.length ?? 0} 家</p></div></div></section>
