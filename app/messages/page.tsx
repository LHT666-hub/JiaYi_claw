...
export default function MessagesPage() {
  const router = useRouter(); const [messages, setMessages] = useState<Message[]>([]); const [channels, setChannels] = useState<Array<Record<string, unknown>>>([]); const [loading, setLoading] = useState(true); const [error, setError] = useState("");
  const load = useCallback(async () => { const response = await fetch("/api/v1/messages", { cache: "no-store" }); const payload = await response.json(); if (response.status === 401) return router.replace("/login"); if (!response.ok) setError(payload.error?.message ?? "消息加载失败"); else { setMessages(payload.data.messages ?? []); setChannels(payload.data.channelBindings ?? []); } setLoading(false); }, [router]);
  useEffect(() => { void load(); }, [load]);
  async function read(item: Message) { if (!item.is_read) { await fetch("/api/v1/messages", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: item.id }) }); setMessages((items) => items.map((message) => message.id === item.id ? { ...message, is_read: true } : message)); } if (item.link_url) router.push(item.link_url); }
  async function readAll() { await fetch("/api/v1/messages", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ markAllRead: true }) }); setMessages((items) => items.map((item) => ({ ...item, is_read: true }))); }
  ...
  return <PhoneShell showBottomNav><div className="space-y-4 px-4 pb-8 pt-7">
    <header className="flex items-end justify-between"><div><p className="text-xs font-semibold text-sage">服务进度与团队通知</p><h1 className="mt-1 text-xl font-semibold text-navy">消息</h1></div>{unread ? <button onClick={() => void readAll()} className="text-sm font-semibold text-sage">全部已读</button> : null}</header>
