...
export default function MessagesPage() {
  ...
  return <PhoneShell showBottomNav><div className="space-y-4 px-4 pb-8 pt-7">
    <ResidentPageHeader title="消息" action={unread ? <button onClick={() => void readAll()} className="rounded-full bg-white px-4 py-2 text-sm font-semibold text-sage">全部已读</button> : null} />
    {isDemo ? <div className="rounded-full border border-sage/20 bg-health-soft px-4 py-2 text-center text-xs font-semibold text-sage">演示数据</div> : null}
    <section className="ios-material mt-5 rounded-[30px] p-4"><div className="flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-health-muted text-sage"><MessageCircleMore className="h-5 w-5" /></div><div><h2 className="text-sm font-semibold text-navy">企业微信家医群</h2><p className="mt-1 text-xs leading-5 text-navy/50">{channels.length ? "已绑定" : "尚未绑定"}</p></div></div></section>
