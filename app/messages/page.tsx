...
export default function MessagesPage() {
  ...
  async function read(item: Message) { if (!item.is_read) { if (!isDemo) await fetch("/api/v1/messages", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: item.id }) }); setMessages((items) => items.map((message) => message.id === item.id ? { ...message, is_read: true } : message)); } if (item.link_url) router.push(item.link_url); }
  async function readAll() { if (!isDemo) await fetch("/api/v1/messages", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ markAllRead: true }) }); setMessages((items) => items.map((item) => ({ ...item, is_read: true }))); }
  ...
  return <PhoneShell showBottomNav><div className="space-y-4 px-4 pb-8 pt-7">
    ...
    {isDemo ? <div className="rounded-full border border-sage/20 bg-health-soft px-4 py-2 text-center text-xs font-semibold text-sage">演示数据 · 已读操作仅在当前页面生效</div> : null}
