...
export default function NotificationSettingsPage() {
  ...
  async function save() {
    setSaving(true);
    try {
      const response = await fetch("/api/v1/notification-preferences", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ serviceUpdates: value.service_updates, followupReminders: value.followup_reminders, contentUpdates: value.content_updates, smsEnabled: value.sms_enabled, wecomEnabled: value.wecom_enabled, quietHoursStart: value.quiet_hours_start.slice(0, 5), quietHoursEnd: value.quiet_hours_end.slice(0, 5) }) });
  ...
  return <PhoneShell showBottomNav><main className="space-y-5 px-4 pb-8"><BackHeader title="通知设置" subtitle="服务结果优先送达，内容通知由您决定。" />
    {loading ? <div className="py-16 text-center text-sm text-navy/45">正在读取通知设置...</div> : <>
      ...
      <section className="ios-material rounded-[30px] p-5"><h2 className="flex items-center gap-2 font-semibold text-navy"><Smartphone className="h-5 w-5 text-sage" />送达渠道</h2><div className="mt-4 space-y-4"><Row label="短信" note="可能产生运营短信费用" checked={value.sms_enabled} onClick={() => toggle("sms_enabled")} /><Row label="企业微信" note="绑定官方家医渠道后可用" checked={value.wecom_enabled} onClick={() => toggle("wecom_enabled")} /></div></section>
