...
export async function POST(request: NextRequest) {
  ...
  try {
    if (parsed.data.entity === "institution") {
      ...
      await writeAuditLog({ actorId: auth.profile.id, action: "care_network.institution_saved", targetTable: "institutions", targetId: data.id, detail: { traceId, networkRole: parsed.data.networkRole }, supabase: auth.supabase });
    ...
    if (parsed.data.entity === "department") {
      const { data, error } = await auth.supabase.from("departments").upsert({ institution_id: parsed.data.institutionId, name: parsed.data.name, specialties: parsed.data.specialties, official_url: parsed.data.officialUrl ?? null }, { onConflict: "institution_id,name" }).select("*").single();
      if (error) throw error;
      await writeAuditLog({ actorId: auth.profile.id, action: "care_network.department_saved", targetTable: "departments", targetId: data.id, detail: { traceId, institutionId: parsed.data.institutionId }, supabase: auth.supabase });
      return apiOk({ department: data }, traceId, 201);
    }
    if (parsed.data.entity === "practitioner") {
      if (parsed.data.departmentId) {
        const { data: department } = await auth.supabase.from("departments").select("id").eq("id", parsed.data.departmentId).eq("institution_id", parsed.data.institutionId).maybeSingle();
        if (!department) return apiError("DEPARTMENT_SCOPE_FORBIDDEN", "医生科室不属于所选机构。", 403, traceId);
      }
      ...
      if (error) throw error;
      await writeAuditLog({ actorId: auth.profile.id, action: "care_network.practitioner_saved", targetTable: "practitioners", targetId: data.id, detail: { traceId, institutionId: parsed.data.institutionId, departmentId: parsed.data.departmentId ?? null }, supabase: auth.supabase });
      return apiOk({ practitioner: data }, traceId, 201);
    }
    ...
    if (parsed.data.toDepartmentId) {
      const { data: department } = await auth.supabase.from("departments").select("id").eq("id", parsed.data.toDepartmentId).eq("institution_id", parsed.data.toInstitutionId).maybeSingle();
      if (!department) return apiError("REFERRAL_DEPARTMENT_MISMATCH", "目标科室不属于上转医院。", 400, traceId);
    }
    ...
    if (error) throw error;
    await writeAuditLog({ actorId: auth.profile.id, action: "care_network.referral_route_saved", targetTable: "referral_routes", targetId: data.id, detail: { traceId, fromInstitutionId: parsed.data.fromInstitutionId, toInstitutionId: parsed.data.toInstitutionId }, supabase: auth.supabase });
    return apiOk({ referralRoute: data }, traceId, 201);
