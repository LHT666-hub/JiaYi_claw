...
export async function POST(request: NextRequest) {
  ...
  const { data: pendingSchedules, error: pendingError } = await auth.supabase
    ...
    .select("id,institution_id,practitioner_id,starts_at,ends_at,source_url,note,practitioner:practitioners(institution_id)")
  ...
  if (parsed.data.decision === "verified") {
    ...
    if (invalid) return apiError("SCHEDULE_VERIFICATION_INCOMPLETE", "排班已过期、医生机构不匹配，或缺少来源/核验说明。", 400, traceId);
  }
