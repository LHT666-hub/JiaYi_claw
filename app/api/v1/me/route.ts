...
import { familyShowcaseMe, residentShowcaseMe } from "@/lib/showcase/resident";
...
export async function GET(request: NextRequest) {
  ...
  if (process.env.NEXT_PUBLIC_DEMO_MODE === "true" && !auth.supabase) {
    return apiOk(auth.profile?.role === "family" ? familyShowcaseMe : {
      ...residentShowcaseMe,
      profile: auth.profile ?? residentShowcaseMe.profile,
    }, traceId);
  }
