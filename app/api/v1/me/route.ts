...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [network, consents, observations, requests, channelBindings, familyBindings] = await Promise.all([
      residentId ? getCareNetworkForResident(residentId, auth.supabase) : Promise.resolve(null),
      ...
      residentId ? auth.supabase.from("health_observations").select("id,observation_type,value,secondary_value,unit,measured_at,source").eq("resident_id", residentId).order("measured_at", { ascending: false }).limit(5) : Promise.resolve({ data: [], error: null }),
    ...
    return apiOk({ profile: auth.profile, residentId, network, consents: consents.data ?? [], observations: observations.data ?? [], serviceRequests: requests.data ?? [], channelBindings: channelBindings.data ?? [], familyBindings: familyBindings.data ?? [] }, traceId);
