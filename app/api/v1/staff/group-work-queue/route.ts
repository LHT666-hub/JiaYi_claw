...
import { staffShowcaseFacts } from "@/lib/showcase/staff";

export async function GET(request: NextRequest) {
  const traceId = createTraceId(); const auth = await getApiAuthContext(request);
  if (!auth.supabase || !auth.profile) {
    if (process.env.NEXT_PUBLIC_DEMO_MODE === "true") return apiOk(staffShowcaseFacts, traceId);
    return apiError("FORBIDDEN", "没有群运营权限。", 403, traceId);
  }
  if (!canAccessWorkbench(auth.profile.role)) return apiError("FORBIDDEN", "没有群运营权限。", 403, traceId);
  const { data, error } = await auth.supabase.from("resident_fact_candidates").select(`
    ...
    resident:profiles!resident_fact_candidates_resident_id_fkey(id,display_name,phone),
