...
export async function GET(_request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase.from("content_items").select("id,category,title,summary,cover_url,original_url,source_name,published_at,effective_from,expires_at,reviewed_at,institution:institutions(name)").eq("id", id).eq("status", "published").maybeSingle();
