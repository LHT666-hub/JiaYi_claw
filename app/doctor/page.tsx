...
function Metric({ label, value, note, icon: Icon, tone }: { label: string; value: number; note: string; icon: typeof ClipboardList; tone: "navy" | "danger" | "amber" | "blue" | "sage" | "purple" }) {
  ...
  return <article className="min-w-0 border border-line bg-white p-3.5 sm:p-4"><div className={`flex h-8 w-8 items-center justify-center rounded-[9px] sm:h-9 sm:w-9 sm:rounded-[10px] ${tones[tone]}`}><Icon className="h-4 w-4" /></div><div className="mt-3 flex items-end justify-between gap-2 sm:block"><p className="text-xl font-semibold tabular-nums sm:text-2xl">{value}</p><p className="truncate text-[10px] text-navy/38 sm:mt-1">{note}</p></div><p className="mt-1 truncate text-xs font-semibold">{label}</p></article>;
}
