...
try {
  ...
  const draftPayload = {
    ...
    fromClaw: false,
    contentId: "",
    idempotencyKey: `release-draft-${Date.now()}`,
  };
  const { data: savedDraft, error: saveDraftError } = await client.rpc("save_service_draft", {
    p_resident_id: userId,
    p_draft_type: "appointment",
    p_payload: draftPayload,
    p_policy_version: "2026-08-11",
  });
  if (saveDraftError || savedDraft?.payload?.target !== draftPayload.target) throw saveDraftError ?? new Error("Service draft was not saved.");
  const { data: loadedDraft, error: loadDraftError } = await client.rpc("load_service_draft", {
    p_resident_id: userId,
    p_draft_type: "appointment",
    p_policy_version: "2026-08-11",
  });
  if (loadDraftError || loadedDraft?.id !== savedDraft.id) throw loadDraftError ?? new Error("Service draft was not recoverable.");
  const { error: revokeDraftConsentError } = await admin.from("consents").update({ granted: false, granted_at: null, revoked_at: new Date().toISOString() })
    .eq("user_id", userId).eq("resident_id", userId).eq("scope", "sensitive_health").eq("policy_version", "2026-08-11");
  if (revokeDraftConsentError) throw revokeDraftConsentError;
  const { count: draftsAfterRevoke, error: draftsAfterRevokeError } = await admin.from("service_drafts").select("id", { count: "exact", head: true }).eq("created_by", userId);
  if (draftsAfterRevokeError || draftsAfterRevoke !== 0) throw draftsAfterRevokeError ?? new Error("Revoking sensitive health consent did not purge service drafts.");
  const { error: restoreDraftConsentError } = await admin.from("consents").update({ granted: true, granted_at: new Date().toISOString(), revoked_at: null })
    .eq("user_id", userId).eq("resident_id", userId).eq("scope", "sensitive_health").eq("policy_version", "2026-08-11");
  if (restoreDraftConsentError) throw restoreDraftConsentError;
  ...
  const { error: deletionDraftError } = await client.rpc("save_service_draft", {
    p_resident_id: userId,
    p_draft_type: "appointment",
    p_payload: draftPayload,
    p_policy_version: "2026-08-11",
  });
  if (deletionDraftError) throw deletionDraftError;
  ...
  const { count: serviceDrafts } = await admin.from("service_drafts").select("id", { count: "exact", head: true }).or(`created_by.eq.${userId},resident_id.eq.${userId}`);
  if (profile?.account_status !== "disabled" || profile?.phone !== null || profile?.display_name !== "已注销用户" || observations !== 0 || legacyProfiles !== 0 || legacyAsks !== 0 || serviceDrafts !== 0) {
    throw new Error(`Deletion anonymization mismatch: ${JSON.stringify({ profile, observations, legacyProfiles, legacyAsks, serviceDrafts })}`);
  }
  ...
  console.log("Verified: notification preferences persisted; service drafts required consent and were purged on revocation/deletion; deletion cooling-off/cancel worked; due deletion anonymized identity and removed current/legacy health data.");
} finally {
